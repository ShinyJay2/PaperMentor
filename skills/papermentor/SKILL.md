...
### Terminal-only UI rule

PaperMentor's interactive navigator must run in a real terminal. Do not render PaperMentor menus, arrow-key screens, numbered choices, or green console simulations inside Codex or Claude chat. Codex/Claude may invoke the CLI, generate HTML blocks, and report the HTML path/evidence, but the user-facing PaperMentor UI is the external terminal plus the browser-rendered `index.html`.

When PaperMentor is invoked from Codex, Claude Code, or any non-interactive shell, open an external terminal and run the PaperMentor command there. Do not use `--codex`, `--show-tui`, transcript snapshots, or chat-inline menu fallback. If an external terminal cannot be opened automatically, fail with a clear terminal-required error instead of printing the TUI into the chat transcript.

### HTML-first reading room rule

Language is part of the launch contract. If the user's PaperMentor request is in a specific language, launch/generate with `--language <code>` when known (`ko`, `en`, `ja`, `ar`, `zh`, etc.) and make the reading guide, Start Here, section menus, and appended HTML blocks use that language as the main prose language while preserving equations, symbols, model names, and natural English technical terms/phrases. If the language is genuinely unknown, leave it as auto and match the source. For non-English output, keep the same structure and depth the English answer would have; translate/localize without dropping displayed equations, term-purpose readings, derivation steps, or figure-specific details.

When Codex or Claude is asked to "open" a PaperMentor room, launch the real terminal UI and open the HTML browser when useful. Never use chat-inline TUI flags or transcript snapshots as the user-facing surface.

On source start, render `index.html` before giving any substantive explanation in the CLI. The user-facing command surface is the Codex/Claude skill invocation: `$papermentor` in Codex and `/papermentor` in Claude Code. The local `papermentor` CLI is a bridge used by the skill to open the terminal UI, maintain sessions, run diagnostics, and update HTML. `papermentor <file-or-url>` opens a guided terminal launch wizard (mode, language, HTML open, generation, reading console) in a real terminal and, from non-interactive shells, reroutes to an external terminal instead of printing inline UI; `papermentor open`, `papermentor go`, `papermentor ask "..."`, `papermentor qa`, and `papermentor export` operate on the active reading room. The first HTML block must be `How to use this reading room`, a compact usage card that explains the linked HTML + CLI/TUI workflow, refresh behavior, and PDF snapshot behavior. The second block must be `Start Here`, not a terminal summary. `Start Here` must contain: (1) a one-sentence model of what the source teaches or claims, (2) the exact representative method/system/algorithm figure crop when present, and (3) a detailed preliminary ladder for concepts needed before reading sections or slides. The CLI must not contain the explanation body; it only shows the launcher, HTML path, detected sections/slides/URL headings, choices, and a place for user questions.

The CLI interaction is a polished, Claude-like branching section/slide/URL-heading navigator. Use the arrow-key TUI (`papermentor tui --session <slug>`) in a real terminal; non-interactive environments must open an external terminal and must not fall back to an inline numbered/chat navigator. `papermentor go` must start from the detected paper sections, slide topics, or URL headings, not from a stale last-selected section. First detect the source mode and its table of contents, sections, slides, or URL headings, then analyze local text before presenting actions. Section actions are **classified by the model on entry — not by word-matching or scoring in the script.** On selecting a section/topic/URL heading, read its title and content, infer its role (a weak position prior — Introduction → Related Work → Method → Experiments → Conclusion, with subsections sharing their parent's role — is only a tiebreak; trust the content over the position), then replace the pending menu with a tailored one via `papermentor section --index <n> --choices "…"`. Match actions to the role (Introduction → promise/core concepts; Related Work → the specific families/citations it actually discusses; Method → its real equations, propositions, assumptions, derivations, pipeline; Experiments → what each result proves; Conclusion → final insight/limitations; URL article sections → the actual claims, examples, code/math, diagrams, caveats, and practical takeaways in that heading) and always include `Ask anything about <section>`. Follow `prompts/section-navigator.md`. The chosen explanation is written to HTML as a new block, never as a long CLI answer. Use `papermentor qa --session <slug>` / `papermentor qa` after adding blocks to score teaching quality and catch shallow summaries, missing proof lines, weak dependency chains, and unresolved figure scaffolds. When an action or question is chosen, hide internal prompt plumbing from normal users: if Codex/Claude automation is available, generate the tailored menu or HTML block automatically and append it to the reading room; otherwise keep the prompt/state files as internal agent handoff artifacts and show a clear provider-unavailable notice that HTML was not changed. Do not show a fake 'preparing' loop when no provider can generate. Do not expose `pending-prompt.md` as a normal user step.

### Representative figure rule

Representative figure choice is a semantic model judgment, not script scoring or word-matching. The script may collect Figure/Fig caption candidates and nearby text, but Codex/Claude must decide whether any candidate is truly the method/system/architecture/mechanism figure; if none is representative, attach no figure. Never attach a full PDF page as a method figure fallback.

In the first paper map, include the exact screenshot/crop of the representative method/system/algorithm/architecture figure from the PDF/page when present. Prefer the figure that explains the method or overall system; do not use experiment/result plots as the representative figure unless no method figure exists. Do not redraw, simplify, generate a substitute diagram, replace it with Mermaid/ASCII/SVG schematics, or use a full-page fallback crop as if it were the representative figure. Attach the captured/cropped figure with `papermentor card --figure-file <path>` or `--figure-url <url>`. Do not use provenance captions such as “Exact crop of …” and do not paste raw PDF-extracted caption text when it contains broken math or hyphenation.
...
Keep installation lightweight: install/copy the skill and CLI first, then diagnose this computer with `papermentor doctor`. Do not hide missing dependencies behind generic environment advice. Report capability tiers explicitly: Core, PDF text, PDF render, Visual, and Export. Use `papermentor doctor --fix` or a targeted fix such as `papermentor doctor --fix poppler` / `papermentor doctor --fix pymupdf` when the user wants automatic repair. Use `papermentor smoke` after installation or repair to prove that a sample HTML reading room can be created and that terminal UI launch support is available.
