...
4. Line transition microscope: for each adjacent pair of proof lines, write a separate subsection with the previous line, next line, reconstructed intermediate lines when needed, and the exact operation(s) that transform one into the other.
...
- Do not use a fixed overview table as the main explanation. Do not output Markdown tables or HTML tables for proof walkthroughs. The main teaching unit is the transition from one displayed line to the next.
- For every transition, infer the operation from the displayed math itself. Do not choose from, imitate, or constrain yourself to a fixed list of operation names; if a table-like checklist comes to mind, rewrite it as prose bullets under the relevant transition.
