...
- Content hierarchy: quiet paper title sheet followed by appended explanation blocks in HTML; the first paper map includes the actual main method figure image when present; blockers, current choices, and diagnostics stay in terminal/state JSON.
...
- Typography: quiet Claude Code-like gothic/sans for paper content; small monospace labels for state and commands; MathJax owns equation typography.
...
## Components
- Existing components to reuse: paper title sheet, paper block, actual paper figure block, and LaTeX block only; no topbar, sidebar, product masthead, or Choose Next chrome in HTML.
