...
- Content hierarchy: quiet paper title sheet followed by appended explanation blocks in HTML; the first paper map includes the exact screenshot/crop of the actual main method figure when present; blockers, current choices, and diagnostics stay in terminal/state JSON.
...
- Typography: Anthropic/Claude-style sans first (`Anthropic Sans`, with system sans fallbacks) for paper content; `Anthropic Mono` first for structured labels; MathJax owns equation typography.
