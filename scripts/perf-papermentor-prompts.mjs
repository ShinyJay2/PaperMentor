...
try {
  ...
  run(['launch', source, '--slug', 'perf-prompt-source', '--auto', '--no-figure', '--no-preview'], {
    ...commonCapture,
    PAPERMENTOR_AGENT_MOCK: startHere
  });
  run(['run', '--session', 'perf-prompt-source', '--index', '4', '--generate'], {
    ...commonCapture,
    PAPERMENTOR_AGENT_MOCK: JSON.stringify([
      'Explain Eq. (1) as patch-aligned representation prediction',
      'Trace why the EMA target encoder stabilizes the objective',
      'Compare representation targets with pixel reconstruction in the method',
