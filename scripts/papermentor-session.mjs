...
function stageQualityRules(type) {
  ...
  const byType = {
    ...
    equation: [
      '- Show the equation before any prose: first output the exact equation from the selected source range as display LaTeX. Preserve variables, subscripts, superscripts, summation/integral limits, conditioning bars, expectation distributions, horizons, and constants. Do not silently simplify, rename, or "clean up" the equation.',
      '- If PDF/text extraction visibly broke the formula, show the closest source snippet first, then write a clearly labeled `Reconstructed formula` only when the reconstruction is mathematically unambiguous from the selected excerpt/context. If it is not unambiguous, say what is unreadable instead of inventing a formula.',
