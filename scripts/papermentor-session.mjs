...
function stageQualityRules(type) {
  ...
  const byType = {
    ...
    equation: [
      '- Start with math evidence, not prose. If the selected source range contains a clean equation, the first block must be that exact equation as display LaTeX. Preserve variables, subscripts, superscripts, summation/integral limits, conditioning bars, expectation distributions, horizons, and constants. Do not silently simplify, rename, or \"clean up\" the equation.',
      '- If PDF/text extraction visibly broke the formula, the first block must be a labeled \`Source snippet (broken extraction)\` display/text block containing the closest source snippet. Only after that may you write a separate clearly labeled \`Reconstructed formula\` display LaTeX block, and only when the reconstruction is mathematically unambiguous from the selected excerpt/context. If it is not unambiguous, say what is unreadable instead of inventing a formula.',
