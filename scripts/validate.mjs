...
for (const phrase of ['renderWelcomeScreen', 'paperMentorMascotLines', 'learningQuote', 'renderPaletteScreen', 'pm <file-or-url>', 'pm open', 'pm ask "question"', 'pm qa']) {
...
function validateSessionHelper() {
  ...
  try {
    ...
    if (!welcomeOutput.includes('PaperMentor') || !sessionScript.includes('const P = `${ansi.green}██${ansi.reset}`') || !welcomeOutput.includes('drop file/url or type a question')) failures.push('pm --snapshot should render the minimal PaperMentor chat launcher');
